...
This package expresses **Understand** as a typed, task-adaptive agent harness. Its concrete capability remains: Analyze a codebase into an interactive knowledge graph of components and relationships. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Understand |
| --- | --- | --- |
| Memory | `entity_relationship_memory` | Retain repository files plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `dependency_and_path_traversal` | Transform symbol extractor into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_change_test_handoff` | Advance the workflow through component model while preserving stop conditions and user authority. |
